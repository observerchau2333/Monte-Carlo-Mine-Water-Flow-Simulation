...
## Why Results May Look Similar Across Runs

Monte Carlo simulations become more stable when the number of samples is large. With `N_DROPS = 1000` and `STEPS = 100`, each run uses 100,000 water drops. The random noise is therefore averaged out, so final results may look similar across runs.

This is usually a good sign: it means the simulation is approaching a stable statistical behavior.

To see stronger randomness, reduce `N_DROPS`, for example:
...
When uploading this project to GitHub, it is usually better not to upload local generated files such as:
...
The output images and CSV files can be uploaded if you want the repository to show example results, but they can also be regenerated by running `simulation.py`.
